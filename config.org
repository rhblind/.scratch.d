...
#+hugo_tags: org-mode
#+hugo_categories: emacs scratch
#+hugo_custom_front_matter: :toc true :aliases /posts/emacs-configuration
#+export_file_name: scratch-config
...
I started with [[https://www.spacemacs.org/][Spacemacs]], then I moved over to [[https://github.com/doomemacs/doomemacs][Doom]]. I liked them both very much and they
...
personal Doom inspired micro-framework called [[https://github.com/rhblind/scratch-emacs][Scratch Emacs]].
...
* Installing Emacs
Emacs can be installed in many ways, but these days I'm usually on a Mac so I just use [[https://brew.sh][Homebrew]].
There's a couple of Emacs packages in Homebrew, but I'm using the [[https://github.com/railwaycat/homebrew-emacsmacport][emacs-mac]] version.
...
See [[https://en.liujiacai.net/2022/09/03/emacs-maxopenfiles/][this]] blog post for a more detailed description.

** Emacs-mac
Installation instructions are in the [[https://github.com/railwaycat/homebrew-emacsmacport/blob/master/README.md][README]] file in the repository, but the TLDR; is something like this.
...
See the [[https://github.com/railwaycat/homebrew-emacsmacport/blob/master/Formula/emacs-mac.rb][emacs-mac.rb]] formula file for a list of supported compilation flags.

* Getting Started
After installing Emacs, follow the installation [[https://github.com/rhblind/scratch-emacs#install][instructions for Scratch Emacs]].
...
*** Emacs server
Start the Emacs server so ~emacsclient~ can connect to this running
instance.
...
~gptel-magit~ commit-message integration below. Backend is a local
Ollama instance running ~qwen2.5-coder:7b~.
...
** ECA
:PROPERTIES:
:ID:       8A6D25E2-5C33-4892-B42D-537B7AF0879B
:END:
Point [[https://github.com/editor-code-assistant/eca-emacs][ECA]] at a self-hosted GLM instance behind an
...
1. Add an entry to ~~/.authinfo.gpg~:
   : machine endpoint.my.api.provider.com login apikey password YOUR_SECRET
2. Run ~scratch sync~ to tangle this file, which writes
   ~~/.config/eca/config.json~ directly and installs the ~eca~ module.

Model keys under ~providers.intility.models~ must be the *Model ID*
(e.g. ~glm-5-3-flash~), not the human-readable *Model name* (e.g. "GLM-5.3
Flash Beta") shown on the deployment dashboard. ECA sends that key
verbatim as the ~model~ field, so the display name gets a 404 "model
does not exist". Same rule applies to ~eca-chat-custom-model~ below.

#+begin_src json :tangle (expand-file-name "eca/config.json" (or (getenv "XDG_CONFIG_HOME") "~/.config")) :mkdirp yes
{
  "providers": {
    "intility": {
      "api": "openai-chat",
      "key": "${env:OPENAI_API_KEY}",
      "url": "https://rolf-local-3dcdaf62-llm.ai.intility.app/v1",
      "models": {
        "GLM-5.2": {},
        "glm-5-3-flash": {}
      }
...
(defun my/eca-set-api-env ()
  ...
  (require 'auth-source)
  (let ((found (car (auth-source-search :host "rolf-local-3dcdaf62-llm.ai.intility.app"
                                        :max 1))))
    (when found
...
(with-eval-after-load 'eca
  (my/eca-set-api-env)
  (setq eca-chat-custom-model "intility/glm-5-3-flash"))

;; ⌘-Return sends the prompt (same context-sensitive behavior as RET) so
;; evil insert mode doesn't force you to ESC first. Evil doesn't bind
;; s-return in insert state, so the key falls through to this mode map.
(with-eval-after-load 'eca-chat
  (define-key eca-chat-mode-map (kbd "s-<return>")
...
(scratch! :editor     (evil +everywhere) leader smartparens
          ...
          :checkers   syntax
          :lang       (org +roam +hugo +pretty +present) markdown csharp elixir erlang json yaml
                      (javascript +deno) likec4
